...
def add_default_products():
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                for product in default_products:
                    ...
                    if cur.fetchone():
                        print(
                            f"⚠️  Product {product['name']} already exists, skipping..."
                        )
                        continue
                    ...
                    print(
                        f"✅ Added product: {product['name']} - ${product['price_usd_cents']/100:.2f}"
                    )
        ...
        print(
            "2. Update the stripe_price_id values in the database with real Stripe price IDs"
        )
