        ...
        try:
            ...
            for line_num, line in enumerate(content.splitlines(), 1):
                for pattern in problematic_patterns:
                    if (
                        pattern in line
                        and "bot_utils." not in line
                        and "def " not in line
                    ):
                        # Skip if calling function within the same module
                        if py_file.name == "bot_utils.py" and pattern.replace(
                            "(", ""
                        ) in [
                            "create_progress_bar",
                            "create_balance_card",
                            ...
                            continue
                        issues.append(
                            f"❌ {py_file}:{line_num} - Missing module prefix: {pattern}"
                        )
...
def validate_webhook_server():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "webhook_server", "src/webhook_server.py"
        )
        if spec is None:
