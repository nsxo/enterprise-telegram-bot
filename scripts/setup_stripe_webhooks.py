...
class StripeWebhookManager:
    ...
    def check_stripe_cli(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                auth_result = subprocess.run(['stripe', 'config', '--list'], 
                                           capture_output=True, text=True)
                if 'test_mode_api_key' in auth_result.stdout or 'live_mode_api_key' in auth_result.stdout:
    ...
    def create_webhook_via_cli(self, local_port: int = 8000) -> None:
        ...
        cmd = [
            'stripe', 'listen',
            '--forward-to', f'localhost:{local_port}/stripe-webhook',
            '--events', events_str
        ]
        
        print(f"Running: {' '.join(cmd)}")
        print("📝 This will display the webhook signing secret - copy it to your .env file as STRIPE_WEBHOOK_SECRET")
...
def main():
    ...
    parser.add_argument(
        '--webhook-url',
        help='Webhook URL (defaults to WEBHOOK_URL from config)'
    )
    ...
    parser.add_argument(
        '--webhook-id',
        help='Webhook ID for delete/test operations'
    )
    
    parser.add_argument(
        '--description',
        help='Description for production webhook'
    )
