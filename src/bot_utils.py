...
from src.config import ADMIN_USER_ID, ADMIN_GROUP_ID
...
def create_enhanced_progress_bar(
    ...
    if max_val is None:
        max_val = int(
            db.get_bot_setting('progress_bar_max_credits') or '100'
        )
...
def create_balance_card(user_data: Dict[str, Any]) -> str:
    ...
    low_threshold = int(
        db.get_bot_setting('balance_low_threshold') or '5'
    )
    critical_threshold = int(
        db.get_bot_setting('balance_critical_threshold') or '2'
    )
